    ...
    <div className="min-h-screen bg-[#f8fafc] text-[#0f172a] flex flex-col relative antialiased selection:bg-[#059669]/10 selection:text-[#059669]">
      ...
      {showAdminCheckInModal && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-[24px] p-6 max-w-sm w-full border border-[#e2e8f0] shadow-2xl space-y-5 animate-in fade-in zoom-in-95 duration-200">
                ...
                <div className="bg-[#ffffff] p-6 rounded-[16px] border border-[#cbd5e1]/60 shadow-sm md:col-span-1">
                  ...
                  <div className="space-y-3 max-h-[350px] overflow-y-auto pr-1">
                    {notifications.length === 0 ? (
                      <p className="text-xs text-slate-500 italic">No security warnings triggered today.</p>
                    ) : (
                      notifications.map(n => (
                        <div key={n.id} className="p-3 bg-rose-50 border border-rose-100 rounded-lg text-xs space-y-1">
              ...
              {showManualLog && (
                <form onSubmit={handleManualAttendance} className="p-4 bg-[#f8fafc] rounded-[16px] border border-[#cbd5e1] grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
                  <div className="flex flex-col space-y-1">
                    ...
                    <select 
                      value={manualProfileId} onChange={(e) => setManualProfileId(target.value)} required // Correcting minor value binding typo
                  ...
                  <div className="md:col-span-3 flex justify-end">
                    <button type="submit" className="bg-[#059669] hover:bg-[#047857] text-white font-bold py-2 px-6 rounded-[5px] shadow transition-all cursor-pointer">
                      Log Correction
                    </button>
                  </div>
                </form>
          ...
          {activeTab === 'settings' && (
            <div className="max-w-xl bg-[#ffffff] p-6 rounded-[16px] border border-[#cbd5e1]/60 shadow-sm space-y-6">
              ...
              <form onSubmit={handleUpdateSettings} className="space-y-4 text-xs">
                ...
                <div className="flex flex-col space-y-1">
                  <label className="font-semibold text-slate-600">Allowable Radius (in Meters)</label>
                  <input 
                ...
                <div className="flex flex-col space-y-1">
                  <label className="font-semibold text-slate-600">Official Office Start Time</label>
                  <input 
